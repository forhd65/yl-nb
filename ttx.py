...
class Spider(Spider):
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        try:
            ...
            # 处理normalList结果
            if data.get('data', {}).get('normalList', {}).get('itemList'):
                for item in data['data']['normalList']['itemList']:
                    try:
                        if item.get('doc', {}).get('id') and len(item['doc']['id']) > 11:
                            video_info = item.get('videoInfo', {})
                            img_tag = video_info.get('imgTag', '{}')
                            try:
                                tag = json.loads(img_tag) if isinstance(img_tag, str) else {}
                            except:
                                tag = {}
                            
                            vlist.append({
                                'vod_id': item['doc']['id'],
                                'vod_name': video_info.get('title', '未知标题'),
                        ...
                        print(f"处理normalList项出错: {e}")
                        continue
            
            # 处理areaBoxList结果
            if data.get('data', {}).get('areaBoxList'):
                for area_box in data['data']['areaBoxList']:
                    if area_box.get('itemList'):
                        for item in area_box['itemList']:
                            try:
                                if (item.get('doc', {}).get('id') and 
                                    len(item['doc']['id']) > 11 and 
                                    item.get('videoInfo', {}).get('title', '').find(key) != -1):
                                ...
                                print(f"处理areaBoxList项出错: {e}")
                                continue
            
            # 处理ipBoxList结果（IP相关内容）
            if data.get('data', {}).get('ipBoxList'):
                for ip_box in data['data']['ipBoxList']:
                    if ip_box.get('itemList'):
                        for item in ip_box['itemList']:
                            try:
                                if item.get('doc', {}).get('id'):
                                    video_info = item.get('videoInfo', {})
                                    vlist.append({
                                        'vod_id': item['doc']['id'],
                                        'vod_name': video_info.get('title') or video_info.get('seriesName', '未知标题'),
                                        'vod_pic': video_info.get('imgUrl') or video_info.get('coverUrl', ''),
                                        'vod_remarks': 'IP内容',
                                        'vod_content': video_info.get('description', '')
                                    })
                            except Exception as e:
                                print(f"处理ipBoxList项出错: {e}")
                                continue
            ...
            unique_vlist = []
            for vod in vlist:
                identifier = vod['vod_id']
                if identifier not in seen:
                    seen.add(identifier)
                    unique_vlist.append(vod)
            
            return {'list': unique_vlist, 'page': pg}
    ...
    def fallback_search(self, key, pg):
        """备用搜索方案"""
        try:
            search_url = f"https://v.qq.com/x/search/?q={key}&stag=0&cur={pg}"
            html = self.gethtml(search_url)
            vlist = []
            
            items = html('.search_result .list_item')
            for item in items.items():
                try:
                    a_tag = item('a.figure')
                    title = a_tag.attr('title') or ''
                    href = a_tag.attr('href') or ''
                    img = item('img').attr('src') or ''
                    desc = item('.desc').text() or ''
                    
                    # 从URL中提取ID
                    cid_match = re.search(r'/x/cover/([^/]+)', href)
                    if cid_match:
